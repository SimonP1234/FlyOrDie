...
void CheckInterferenceAndSwitch()
{
    static uint8_t goodPacketStreak = 0;
    ...
    if (LQCalc.currentIsSet())
    {
        ...
        goodPacketStreak++;
        if (goodPacketStreak >= 3)
        {
            // Reset bad packet counter after 3 consecutive good packets
            consecutiveBadPackets = 0;
            goodPacketStreak = 0;
        }
    }
    else
    {
        ...
        // Trigger domain switch if threshold reached
        if (consecutiveBadPackets >= DOMAIN_SWITCH_THRESHOLD)
        {
            DBGLN("Interference detected! LQ=%u, switching domain...", 
                  linkStats.uplink_Link_quality);
            domainSwitchPending = true;
        }
    }
}
...
bool ICACHE_RAM_ATTR ProcessRFPacket(SX12xxDriverCommon::rx_status const status)
{
    ...
    bool crcGood = OtaValidatePacketCrc(otaPktPtr);

    #if !defined(DISABLE_ANTI_JAMMING)
    anti_jamming_register_packet(crcGood ? 1 : 0, millis());
    #endif
...
void setup()
{
    ...
    else
    {
        ...
        #if !defined(DISABLE_ANTI_JAMMING)
        {
            // Allocate buffer for anti-jamming context
            aj_config_t aj_cfg = {0};
            aj_cfg.window_size_packets = 100;
            aj_cfg.window_duration_ms = 1000;
            aj_cfg.window_mode = AJ_WINDOW_BY_COUNT;
            aj_cfg.jam_threshold_percent = 30; // 30% bad packets = interference
            aj_cfg.min_bad_packets = 5;
            aj_cfg.consecutive_windows_to_jam = 2;
            aj_cfg.jam_state_hold_time_ms = 2000;
            aj_cfg.min_time_between_reco_ms = 500;
            aj_cfg.allow_group_switch_suggestions = 1;
            
            size_t aj_size = aj_context_size_bytes(&aj_cfg);
            static uint8_t aj_buffer[512]; // Static buffer
            
            if (aj_size <= sizeof(aj_buffer)) {
                aj_ctx_t* ctx = anti_jamming_init_with_buffer(aj_buffer, sizeof(aj_buffer), &aj_cfg);
                if (ctx) {
                    DBGLN("Anti-jamming initialized (window=%u, threshold=%u%%)", 
                        aj_cfg.window_size_packets, aj_cfg.jam_threshold_percent);
                    
                    // Initialize RC switch control
                    anti_jamming_switch_init();
                    DBGLN("Anti-jamming RC switch (CH5/CH7) enabled");
                }
            }
        }
        #endif
